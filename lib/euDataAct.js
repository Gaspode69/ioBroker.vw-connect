...
function loadSemanticDictionary() {
  if (_SEMANTICS) return _SEMANTICS;
  try {
    let raw = fs.readFileSync(path.join(__dirname, "euDataActSemantics.json"), "utf-8");
    if (raw.charCodeAt(0) === 0xfeff) raw = raw.slice(1);
    _SEMANTICS = JSON.parse(raw);
  } catch {
    _SEMANTICS = { points: {}, reports: {} };
  }
  return _SEMANTICS;
}
...
function normalizeDataset(payload) {
  ...
  for (const fieldName of Object.keys(points)) {
    ...
    const value = parseDatasetItemValue(item, meta);
...
function parseDatasetItemValue(item, meta) {
...
/**
 * Preserve the EU Data Act payload as dictionary-aware data points keyed by
 * their immutable UUIDs. This intentionally avoids the old dataFieldName-only
 * flattening so duplicate fields, report timestamps and report-specific
 * descriptions remain inspectable by adapter users.
 */
function buildSemanticDatasetTree(payload) {
  const dictionary = loadDictionary();
  const semantics = loadSemanticDictionary();
  const out = {
    _meta: {
      vin: payload.vin || null,
      user_id: payload.user_id || null,
  ...
  for (const [index, item] of (payload.Data || []).entries()) {
    if (!item || !item.key) continue;
    const meta = dictionary[item.key] || {};
    const semantic = semantics.points[item.key] || null;
    const fieldName = item.dataFieldName || meta.name || item.key;
    const value = parseDatasetItemValue(item, meta);
    const point = {
      value,
      rawValue: item.value == null ? null : String(item.value),
    ...
    if (semantic) {
      ...
      const report = semantic.report || "unknown";
      const role = semantic.role || "value";
      if (!out.semanticIndex.reports[report]) {
        out.semanticIndex.reports[report] = {
          _meta: semantics.reports[report] || {},
        };
      }
      addSemanticPoint(out.semanticIndex.reports[report], role, point);
    }

    for (const cluster of clusterNames(meta.cluster)) {
      addClusterPoint(out.clusters, cluster, fieldName, point);
    }
  }
...
function addClusterPoint(clusters, cluster, fieldName, point) {
  const fieldNode = getOrCreateNested(clusters, `${cluster}.${fieldName}`);
  fieldNode.entries = fieldNode.entries || [];
  fieldNode.entries.push(point);
  fieldNode.duplicateCount = fieldNode.entries.length;

  const primary = pickPrimaryPoint(fieldNode.entries);
  Object.assign(fieldNode, primary);
}

function addSemanticPoint(reportNode, role, point) {
  const roleNode = getOrCreateNested(reportNode, role);
  roleNode.entries = roleNode.entries || [];
  roleNode.entries.push(point);
  roleNode.duplicateCount = roleNode.entries.length;

  const primary = pickPrimaryPoint(roleNode.entries);
  Object.assign(roleNode, primary);
}

function pickPrimaryPoint(points) {
  return points.reduce((best, point) => (point.key < best.key ? point : best), points[0]);
}

function clusterNames(rawCluster) {
  const raw = String(rawCluster || "").trim();
  if (!raw) return ["Unclustered"];
  return raw
    .split(",")
    .map((part) => part.trim())
    .filter(Boolean);
...
function getOrCreateNested(target, dottedName) {
